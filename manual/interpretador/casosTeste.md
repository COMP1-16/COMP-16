...
> Legenda: **V** = Válidos, **I** = Inválidos, **Sint.** = Sintático, **Sem.** = Semântico, **Exec.** = Execução
...
| `valido_dce_pos_inc_loop` | `int i = 0; while (0) i++; i++;` | `—` |
| `valido_fold_atrib_mult` | `int x = 2; x *= 3 + 4;` | `—` |
| `valido_fold_atrib_soma` | `int x = 1; x += 2 + 3;` | `—` |
| `valido_fold_atrib_sub` | `int x = 10; x -= 4 + 1;` | `—` |
| `valido_inc_sequencia` | `int i = 0; i++; i++; i++; printf("%d", i);` | `3` |
| `valido_mult_atrib` | `int x = 3; x *= 4; printf("%d", x);` | `12` |
| `valido_noop_atrib_zero` | `int x = 5; x += 0;` | `—` |
...
| `valido_decl_multipla_fold` | `int x = 10, y = 2 + 3;` | `—` |
| `valido_decl_multipla_print` | `int x = 3, y = 4; printf("%d", x + y);` | `7` |
| `valido_decl_sem_uso` | `int x = 1 + 1;` | `—` |
| `valido_fold_init_expr` | `int a = 1; int b = a + 4;` | `—` |
| `valido_fold_init_int` | `int x = 2 + 3;` | `—` |
| `valido_init_float_fold` | `float f = 1.0 + 2.0;` | `—` |
...
| `valido_for_break_dce` | `int x = 0; for (;;) { break; x = 5; }` | `—` |
| `valido_for_cond_fold` | `int x = 0; for (int i = 0; i < 1 - 1; i++) x = 1;` | `—` |
| `valido_for_contagem_regressiva` | `int n = 3; for (; n > 0; n--) { } printf("%d", n);` | `0` |
| `valido_for_corpo_morto` | `int x = 0; for (int i = 0; i < 0; i++) x = 99;` | `—` |
| `valido_for_corpo_sem_chaves` | `int i = 0; for (i = 0; i < 2; i++) i++; printf("%d", i);` | `2` |
| `valido_for_inc_fold` | `int x = 0; for (int i = 0; i < 0; i = i + 1 + 1) x = 1;` | `—` |
| `valido_for_init_decl` | `int s = 0; for (int i = 0; i < 2; i++) s++; printf("%d", s);` | `2` |
| `valido_for_init_fold` | `int x = 0; for (int i = 0 + 0; i < 0; i++) x = 1;` | `—` |
...
| Caso | Entrada |
|------|---------|
| `valido_else` | `int x=0; if(x){} else{x=1;}      // OK — x acessível e mutável no else` |
| `valido_if` | `int x=1; if (x) { }  // 	OK — inteiro usado como condição (conversão implícita)` |
| `valido_if_com_declaracao` | `int x; if (x>0) {int y=x;}  // OK — y declarado e usado dentro do escopo do bloco` |
...
| `valido_else_morto` | `int x = 0; if (1) { x = 1; } else { x = 2; }` | `—` |
| `valido_if_aninhado_fold` | `int x = 0; if (0) { if (1) { x = 1; } }` | `—` |
| `valido_if_cond_fold` | `int x = 0; if (1 - 1) { x = 1; }` | `—` |
...
| `valido_if_um_dce` | `int x = 1; if (1) { x = 2; } else { x = 99; }` | `—` |
| `valido_if_zero_dce` | `int x = 1; if (0) { x = 99; } else { x = 2; }` | `—` |
...
| Caso | Entrada |
|------|---------|
| `invalido_math_funcao_sem_include` | `float a = sqrt(16.0); // Erro semântico: sqrt não foi declarado na tabela (falta o #inc...` |
| `invalido_math_parametros_faltando` | `#include <math.h>  float a = pow(2.0); // Erro semântico/Runtime: pow espera 2 argument...` |

### Execucao — Validos

| Caso | Entrada | Saida esperada |
|------|---------|----------------|
| `valido_abs_literal` | `#include <math.h> float x = abs(-5.0);` | `—` |
| `valido_floor_ceil` | `#include <math.h> float a = floor(1.7); float b = ceil(1.2);` | `—` |
...
| `valido_pow_literal` | `#include <math.h> float x = pow(2.0, 3.0);` | `—` |
| `valido_round_literal` | `#include <math.h> float x = round(2.5);` | `—` |
| `valido_sqrt_literal` | `#include <math.h> float x = sqrt(16.0);` | `—` |
...
| Caso | Entrada | Saida esperada |
|------|---------|----------------|
| `valido_atrib_op_fold` | `int x = 1; x = x + 2 + 3;` | `—` |
| `valido_div_inteira_print` | `printf("%d", 10 / 3);` | `3` |
| `valido_fold_div_soma` | `int x = 10 / 2 + 1;` | `—` |
| `valido_fold_float` | `float f = 1.5 + 2.5;` | `—` |
| `valido_fold_precedencia` | `int x = 2 + 3 * 4;` | `—` |
| `valido_fold_sub` | `int x = 10 - 3 - 2;` | `—` |
...
| `invalido_recursao_nome` | `// invalido_recursao_nome.txt // ERRO ESPERADO: Erro no Semântico (tempo de checagem), ...` |
...
| `valido_recursao_fatorial` | `// 01_fatorial.txt // Calcula o fatorial de um número N de forma recursiva  int fat(int...` |
| `valido_recursao_fibonacci` | `// 02_fibonacci.txt // Calcula o N-ésimo número da sequência de Fibonacci  int fib(int ...` |
| `valido_recursao_mdc` | `// 03_mdc.txt // Calcula o Máximo Divisor Comum (MDC) usando o algoritmo de Euclides  i...` |
...
|------|---------|----------------|-----------|
| `valido_atof_fold` | `#include <stdlib.h> float x = atof("1.0") + 0.0;` | `—` | `—` |
| `valido_atof_literal` | `#include <stdlib.h> float x = atof("3.14");` | `—` | `—` |
| `valido_atoi_expr` | `#include <stdlib.h> int x = atoi("10") + 0;` | `—` | `—` |
| `valido_atoi_literal` | `#include <stdlib.h> int x = atoi("42");` | `—` | `—` |
| `valido_exit_fold` | `#include <stdlib.h> if (0) exit(1 + 2);` | `—` | `—` |
| `valido_stdlib_exit_0` | `#include <stdlib.h>  exit(0);` | `—` | `0` |
| `valido_stdlib_exit_1` | `#include <stdlib.h>  exit(1);` | `—` | `1` |
| `valido_stdlib_valores` | `#include <stdlib.h>  printf("%d\n", atoi("-17")); printf("%f\n", atof("2.5"))...` | `-17 2.500000 100` | `—` |
...
| Caso | Entrada | Saida esperada |
|------|---------|----------------|
| `valido_dce_after_break` | `int x = 2;     int y;     switch (x) {     case 1:         break;         y =...` | `—` |
...
| `valido_while_break_dce` | `int x = 0; while (1) { break; x = 5; }` | `—` |
| `valido_while_cond_fold` | `int x = 0; while (1 - 1) x = 1;` | `—` |
| `valido_while_contador` | `int i = 0; while (i < 3) i++; printf("%d", i);` | `3` |
| `valido_while_corpo_morto` | `int x = 0; while (0) { x = 1; x = 2; }` | `—` |
...
| `valido_while_literal_fold` | `int x = 0; while (2 - 2) x = 1;` | `—` |
...
| 2.0 | 21/06/26 | Reestruturação dos casos de teste | Camila Careli |