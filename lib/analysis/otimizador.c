...
static No *eliminarDeadCodeAposBreakOuContinue(No *no){
    if (!no || no->tipo != NO_BLOCO) return no;
    int before_dead_idx = -1;
    for (int i = 0; i < no->u.bloco.count; i++) {
        if (no->u.bloco.stmts[i]->tipo == NO_BREAK || no->u.bloco.stmts[i]->tipo == NO_CONTINUE) {
            before_dead_idx = i;
            break;
        }
    }
    if (before_dead_idx < 0) return no;

    for (int i = before_dead_idx + 1; i < no->u.bloco.count; i++)
        liberarNo(no->u.bloco.stmts[i]);

    no->u.bloco.count = before_dead_idx + 1;
    return no;
};
        ...
        case NO_SWITCH:
            ...
            if (no->u.switch_stmt.cases) {
                for (int i = 0; i < no->u.switch_stmt.cases->u.bloco.count; i++) {
                    ...
                    switch (c->tipo) {
                        case NO_CASE_INT:
                            c->u.case_int.stmts = eliminarDeadCodeAposBreakOuContinue(otimizar(c->u.case_int.stmts));
                            break;
                        case NO_CASE_CHAR:
                            c->u.case_char.stmts = eliminarDeadCodeAposBreakOuContinue(otimizar(c->u.case_char.stmts));
                            break;
                        case NO_DEFAULT:
                            c->u.case_default.stmts = eliminarDeadCodeAposBreakOuContinue(otimizar(c->u.case_default.stmts));
                            break;
                        default:
                            break;
                    }
